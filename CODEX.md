...
├── log-session.md                 # Session 日志
├── docs/                          # 项目文档
├── eval_reports/                  # 评估报告（HTML）
└── pkg/                           # 打包后的提交包
```
...
| 用途 | 远程路径 |
|------|---------|
| Session 日志 | `/data/app/sandbox/kaggle/kg-rl/log-session.md` |
| 训练日志 | `/data/app/sandbox/kaggle/kg-rl/log` |
| 项目文档 | `/data/app/sandbox/kaggle/kg-rl/docs` |
| 评估报告 | `/data/app/sandbox/kaggle/kg-rl/eval_reports/` |
| 提交包 | `/data/app/sandbox/kaggle/kg-rl/pkg/` |
...
> ⚠️ 评估在**远程服务器**上运行，报告保存到远程路径。

| 属性 | 值 |
|------|-----|
| 评估脚本位置 | 远程 `/data/app/sandbox/kaggle/kg-rl/scripts/` |
| 报告保存路径 | `/data/app/sandbox/kaggle/kg-rl/eval_reports/` |
| 报告格式 | HTML（包含训练曲线、Win Rate 对比等） |
| 同步到本地 | 需手动从远程复制，或在远程生成后直接查看 |
